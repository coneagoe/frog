...
const props = { busyTargetIds: new Set<number>(), loading: false, page: 2, totalCount: 75, totalPages: 2, onPreviousPage: vi.fn(), onNextPage: vi.fn(), onEdit: vi.fn(), onToggle: vi.fn(), onDelete: vi.fn() };
...
describe("MonitorTargetTable", () => {
  ...
  it("shows actions only in management mode and disables a busy row", () => {
    render(<MonitorTargetTable {...props} busyTargetIds={new Set([target.id])} managementMode targets={[target]} />);
