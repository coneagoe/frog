...
每日工作流使用既有的合格业绩预告、活跃黑屋过滤和既有 SSF 检测器生成监控候选，并将业绩预告、股东、黑屋及目标的审计证据保存到 `forecast_ssf_candidates` 表。候选状态包括 `eligible`、`deferred`、`blackroom` 和 `paused`，退休是非破坏性的，审计记录会保留。
