...
认证用户可通过 Paper Trading 控制台的 `/monitor` 页面查看统一监控目标。页面以 `GET /paper/monitor-targets` 为唯一主数据源，一次返回手工和工作流目标的 `items`、`summary` 和 `pagination`；每项包含 `target_type`、`can_manage`、运行状态、条件/工作流及最近检查信息。默认只读；进入“管理模式”后才显示创建入口和操作列，且仅 `can_manage=true` 的手工目标可创建、编辑、启用/禁用或永久删除，工作流目标始终只读。

主接口支持按市场、频率和启用状态筛选；`condition_type` 是手工目标专用条件筛选，使用它时结果不包含工作流目标。`sort` 由后端执行，支持默认顺序、股票代码升序和降序。页面大小仅支持 25、50 或 100。后端返回规范化页码：请求超出范围时返回最后一页（无结果时为第 1 页），并在 `pagination` 中返回实际 `page`、`page_size`、`total_count` 和 `total_pages`。`summary` 的 running、paused、disabled、triggered、daily、intraday 及 total 均统计全部筛选结果，而非当前页。

股票名称按“市场 + 股票代码”解析；无法解析或名称为空时为 `null`，界面显示占位符，不能仅按代码跨市场复用名称。页面展示价格/指标告警提示，明确不会执行任何交易；加载初次显示加载状态，刷新期间保留已有数据并显示更新状态，失败提供安全错误信息和 Retry 操作。筛选、排序或页大小变更回到第 1 页；无匹配结果显示筛选提示，无任何目标时显示空状态。表格在窄屏可横向滚动，筛选器逐步收窄为两列/单列，管理操作允许换行。

主接口中的运行健康字段仍为只读：状态优先级为 `disabled` > `paused` > `running`，`triggered` 独立于 `last_state`；最后检查时间仅表示已完成且数据并非不足的运行器评估，配置变更、同步和刷新不会伪造该时间。错误仅展示固定安全摘要及脱敏、截断的详情，不展示原始错误。旧的 `GET /paper/monitor-targets/manual` 和 `GET /paper/monitor-targets/health` 仅作为 deprecated 兼容层保留，新的客户端应使用主接口。控制台不提供暂停或恢复操作；工作流专用的 pause/resume 行为和 CLI 命令保持不变。编辑目标使用右侧滑出面板，创建和编辑均提供逐字段校验。
...
投递记录状态及含义如下：`pending` 表示等待投递或下一次重试；`processing` 表示已被投递工作者租约领取；`delivered` 表示投递成功的终态；`failed` 表示达到重试上限后的失败终态；`cancelled` 表示目标删除或取消后抑制投递。失败按现有的有界指数退避计划重试：第 1 至第 4 次失败后分别等待 1、2、4、8 分钟，第 5 次失败后进入 `failed`，每分钟投递任务仍按既有计划运行。
