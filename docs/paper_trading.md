...
`unavailable_dates`, `failed_dates`, and `errors`. Event/list money is displayed
to 4 decimal places and quantities to 6 decimal places; this is serialization
only and does not reduce internal precision.
...
The public serializers round money to 4 decimal places and quantities to 6
decimal places. Consumers must use stored/internal values for reconciliation,
not the shorter display strings.

Events are listed and replayed in deterministic `(event_at, id)` order. The
account plus `idempotency_key` is unique. Replaying the same key with the same
