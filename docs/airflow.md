...
恢复按证券缺口幂等执行：已有精确 BFQ 数据的缺口会被标记为已解决，未解决缺口可独立重试，不会重复写入已恢复的行情行。每次批次、尝试及缺口摘要都会保留 routing/classification 和 provider/结果证据，便于审计和重试。账户修复、审批以及升级处置不属于本次自动恢复流程，仍是后续独立边界。
