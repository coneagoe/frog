...
def download_history_data_stock_hk(stock_id: str, period: str, start_date: str, end_date: str, adjust="hfq"):
    ...
    if not os.path.exists(data_path):
        df = ak.stock_hk_hist(symbol=stock_id, period=period,
                              start_date=start_date.replace('-', ''),
                              end_date=end_date.replace('-', ''),
                              adjust=adjust)
        df = df.rename(columns={
            '日期': COL_DATE, '开盘': COL_OPEN, '收盘': COL_CLOSE,
            '最高': COL_HIGH, '最低': COL_LOW, '成交量': COL_VOLUME,
        })
        df[COL_DATE] = pd.to_datetime(df[COL_DATE])
        df.to_csv(data_path, encoding='utf_8_sig', index=False)
        return
    ...
    end_date_ts1 = pd.Timestamp(datetime.today().strftime('%Y-%m-%d'))
    try:
        df0 = ak.stock_hk_hist(symbol=stock_id, period=period,
                               start_date=start_date_ts1.strftime('%Y%m%d'),
                               end_date=end_date_ts1.strftime('%Y%m%d'),
                               adjust=adjust)
    ...
    df0[COL_DATE] = pd.to_datetime(df0[COL_DATE])
    df = pd.concat([df, df0], ignore_index=True).sort_values(by=COL_DATE).drop_duplicates(subset=[COL_DATE])
    df.to_csv(data_path, encoding='utf_8_sig', index=False)
