...
def _create_legacy_schema(connection: Connection) -> None:
    statements = (
        "CREATE TABLE paper_accounts (id integer generated by default as identity primary key, status varchar(20) NOT NULL DEFAULT 'active', fee_preset varchar(30) NOT NULL DEFAULT 'a_share', migration_repair_reason varchar(40))",
        ...
        "CREATE TABLE paper_position_round_trips (id integer primary key, status varchar(20) NOT NULL DEFAULT 'open', market varchar(20) NOT NULL DEFAULT 'a_share')",
        ...
        "CREATE INDEX ix_paper_position_round_trips_market ON paper_position_round_trips (market)",
...
def test_postgresql_storage_startup_preserves_governed_paper_storage_boundary(postgres_schema, monkeypatch):
    ...
    try:
        ...
        with _connection(engine, schema) as connection:
            ...
            forecast_snapshot_runs = connection.execute(
                text("SELECT to_regclass('forecast_snapshot_runs')")
            ).scalar_one()
    ...
    assert forecast_snapshot_runs == "forecast_snapshot_runs"
