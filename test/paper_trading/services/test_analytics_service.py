...
def test_analytics_computes_execution_and_trade_quality(tmp_path):
    ...
    repo.create_order(
        account.id, "000001.SZ", OrderSide.BUY, 100,
        Decimal("10.00"), date(2026, 6, 16), OrderStatus.FILLED,
    )
    repo.create_order(
        account.id, "000002.SZ", OrderSide.BUY, 100,
        Decimal("10.00"), date(2026, 6, 16), OrderStatus.REJECTED,
        rejection_code="INSUFFICIENT_CASH",
        rejection_reason="Insufficient cash",
    )
    win = repo.create_round_trip(
        account.id, "000001.SZ", 1, date(2026, 6, 16),
        Decimal("1000.0000"), Decimal("5.0000"),
    )
...
def test_analytics_computes_total_return_and_drawdown(tmp_path):
    ...
    repo.save_snapshot(
        account_id=account.id, trade_date=date(2026, 6, 16),
        cash_available=Decimal("100000.0000"), cash_frozen=Decimal("0"),
        market_value=Decimal("0"), total_assets=Decimal("100000.0000"),
        realized_pnl=Decimal("0"), unrealized_pnl=Decimal("0"),
        position_count=0, order_count=0, trade_count=0,
    )
    repo.save_snapshot(
        account_id=account.id, trade_date=date(2026, 6, 17),
        cash_available=Decimal("110000.0000"), cash_frozen=Decimal("0"),
        market_value=Decimal("0"), total_assets=Decimal("110000.0000"),
        realized_pnl=Decimal("0"), unrealized_pnl=Decimal("0"),
        position_count=0, order_count=0, trade_count=0,
    )
    repo.save_snapshot(
        account_id=account.id, trade_date=date(2026, 6, 18),
        cash_available=Decimal("99000.0000"), cash_frozen=Decimal("0"),
        market_value=Decimal("0"), total_assets=Decimal("99000.0000"),
        realized_pnl=Decimal("0"), unrealized_pnl=Decimal("0"),
        position_count=0, order_count=0, trade_count=0,
    )
