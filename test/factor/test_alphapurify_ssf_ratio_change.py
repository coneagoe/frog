...
import factor.alphapurify_ssf_ratio_change as alphapurify_ssf_ratio_change  # noqa: E402
...
def _make_price_history() -> pd.DataFrame:
    return pd.DataFrame(
        {
            COL_DATE: pd.bdate_range("2024-04-01", periods=5),
...
def test_main_loads_ssf_ratio_change_panel_and_writes_report(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(alphapurify_ssf_ratio_change, "get_storage", lambda: storage)
