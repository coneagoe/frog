...
import factor.alphapurify_ssf_ratio as alphapurify_ssf_ratio  # noqa: E402
...
def test_main_loads_ssf_ratio_panel_and_writes_report(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(alphapurify_ssf_ratio, "get_storage", lambda: storage)
    ...
    monkeypatch.setattr(alphapurify_ssf_ratio, "FactorAnalyzer", FakeFactorAnalyzer)

    output_path = tmp_path / "ssf_ratio_ic.html"
    code = alphapurify_ssf_ratio.main(["--max-stocks", "1", "--report-html", str(output_path)])
