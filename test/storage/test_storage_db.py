...
class TestStorageDb:
    ...
    def test_save_history_data_fund_normalizes_code_and_trade_date(self, storage_db):
        ...
        written_df = cast(pd.DataFrame, captured["df"])
    ...
    def test_save_history_data_hk_stock_unsupported_period_returns_false(self, storage_db):
        ...
        mock_logger.error.assert_called_once()
        assert "不支持的港股数据周期" in mock_logger.error.call_args[0][0]
    ...
    def test_load_history_data_fund_builds_expected_sql_and_params(self, storage_db):
        with patch("storage.storage_db.pd.read_sql") as mock_read_sql:
            ...
            assert f'"{COL_DATE}" <= %s' in sql
            assert f'ORDER BY "{COL_DATE}" ASC' in sql
            assert kwargs["params"] == ("510300", "2024-01-01", "2024-03-31")
        ...
        with engine.begin() as connection:
            ...
            connection.execute(
                text(
                    ...
                    "ts_code varchar(32) NOT NULL, announcement_date date NOT NULL, report_end_date date NOT NULL, "
                    "forecast_type varchar(20) NOT NULL, growth_min double precision, growth_max double precision, "
