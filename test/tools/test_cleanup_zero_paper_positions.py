...
def test_cleanup_backfills_account_pnl_and_removes_closed_positions():
    ...
    repo.upsert_position(account.id, "CLOSED", 0, 0, Decimal("0"), realized_pnl=Decimal("75"))
    repo.upsert_position(account.id, "OPEN", 100, 0, Decimal("1000"), realized_pnl=Decimal("20"))
...
def test_cleanup_rejects_frozen_closed_position_without_writes():
    ...
    repo.upsert_position(account.id, "FROZEN", 0, 1, Decimal("0"), realized_pnl=Decimal("75"))
