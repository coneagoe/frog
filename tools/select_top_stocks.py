...
def main():
    ...
    parser.add_argument(
        "-e", "--end", required=False, help="End date in YYYY-MM-DD format"
    )
    ...
    open_baseline = load_history_data(
        security_id=args.baseline,
        period="daily",
        start_date=args.start,
        end_date=args.start,
    )[COL_OPEN].values[0]
    close_baseline = load_history_data(
        security_id=args.baseline,
        period="daily",
        start_date=args.end,
        end_date=args.end,
    ...
    df[COL_OPEN] = df[COL_STOCK_ID].apply(
        lambda x: load_history_data(
            security_id=x, period="daily", start_date=args.start, end_date=args.start
        )[COL_OPEN].values[0]
    )
    df[COL_CLOSE] = df[COL_STOCK_ID].apply(
        lambda x: load_history_data(
            security_id=x, period="daily", start_date=args.end, end_date=args.end
        )[COL_CLOSE].values[0]
    )

    df["涨幅"] = (df[COL_CLOSE] - df[COL_OPEN]) / df[COL_OPEN] * 100
    df["涨幅"] = df["涨幅"].round(2)
    df = df[df["涨幅"] > increase_baseline]
    df = df[df["涨幅"] > 1]
