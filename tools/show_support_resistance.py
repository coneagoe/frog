...
def get_target_prices(stock_id: str):
    df = pd.read_excel(trading_book_path, sheet_name="持仓", dtype={COL_STOCK_ID: str})
    ...
    try:
        ...
        tp0 = df.loc[df[COL_STOCK_ID] == stock_id]["目标价格1"].iloc[0]
        tp1 = df.loc[df[COL_STOCK_ID] == stock_id]["目标价格2"].iloc[0]
        tp2 = df.loc[df[COL_STOCK_ID] == stock_id]["目标价格3"].iloc[0]
    except IndexError:
        pass
...
def draw_support_resistance(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=df[COL_CLOSE].iloc[turning_points],
            mode="markers",
            name="拐点",
        )
    )
    ...
    if resistance_point:
        ...
        fig.add_annotation(
            x=df[COL_DATE].iloc[resistance_point],
            y=df[COL_CLOSE].iloc[resistance_point],
    ...
    if support_point:
        ...
        diff_percent = round(diff / current_price * 100, 2)
        fig.add_annotation(
            x=df[COL_DATE].iloc[support_point],
            y=df[COL_CLOSE].iloc[support_point],
...
def usage():
    print(
        f"{os.path.basename(__file__)} [-n days] [-s start_date] [-e end_date] <stock_id>"  # noqa: E501
    )
    print(f"{os.path.basename(__file__)} -h")
    print(
        "-n days: since how many days ago, cannot be used together with -s and -e. default is 120"  # noqa: E501
    )
