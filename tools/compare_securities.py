...
def show_multiple_securities(security_ids, df_list):
    ...
    for i in range(1, len(df_list)):
        merged_df = pd.merge(
            merged_df, df_list[i], on=COL_DATE,
            suffixes=('', f'_{i}')
        )
    ...
    for i, sec_id in enumerate(security_ids):
        ...
        fig.add_trace(go.Scatter(
            x=merged_df[COL_DATE], y=merged_df[norm_close_col],
            mode='lines', name=get_security_name(sec_id)
        ))

    # Add average trace
    fig.add_trace(go.Scatter(x=merged_df[COL_DATE], y=avg_close,
...
if __name__ == "__main__":
    ...
    for sec_id in security_ids:
        df_list.append(load_history_data(security_id=sec_id, period='daily',
