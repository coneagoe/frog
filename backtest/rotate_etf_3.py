...
class RotateStrategy(bt.Strategy):
    ...
    def __init__(self):     # noqa: E303
        ...
        self.ema_20 = {i: bt.indicators.EMA(self.datas[i].low,
                                            period=20)
                                            for i in range(len(self.datas))}
    ...
    def next(self):    # noqa: E303
        ...
        for i in range(len(self.datas)):
            if gContext[i].order:
                ...
                if (i not in selected and gContext[i].holding_bars >= self.params.holding_bars) or \
...
if __name__ == "__main__":
    ...
    run(strategy_name='rotate_etf_3', cerebro=cerebro, stocks=stocks,
