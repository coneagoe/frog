...
def load_test_data(
    ...
    # Convert date column to pandas Timestamp to ensure compatibility with backtrader
    df["date"] = pd.to_datetime(df["date"])

    start_date_dt = pd.to_datetime(start_date).date()
    date_from_df = df.iloc[0]["date"]
    df_date = date_from_df.date()

    if start_date_dt < df_date:
        ...
        # Ensure both DataFrames have the same index type (keep as Timestamp for backtrader compatibility)
