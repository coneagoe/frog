        ...
        for i in range(len(self.datas)):
            if gContext[i].order:
                ...
                if (i not in selected) and \
                    (gContext[i].holding_bars >= self.params.holding_bars or self.datas[i].close[0] < gContext[i].stop_price):
...
if __name__ == "__main__":
    ...
    run(strategy_name='rotate_etf_1', cerebro=cerebro, stocks=stocks,
