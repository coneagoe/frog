...
from .bt_common import run, show_position
        ...
        for i in range(len(self.datas)):
            if gContext[i].order:
                ...
                if (i not in selected) and (
                    gContext[i].holding_bars >= self.p.holding_bars or self.datas[i].close[0] < gContext[i].stop_price
                ):
