...
class ObosStrategy(MyStrategy):
    ...
    def next(self):
        ...
        to_open_stock_count = 0
        holding_stock_count = 0
        bad_performance_stocks = []
        ...
        for i in range(len(self.datas)):
            if self.context[i].order_state == OrderState.ORDER_IDLE:
                ...
                if self.context[i].order_state == OrderState.ORDER_HOLDING:
                    holding_stock_count += 1
                    if self.context[i].profit_rate < -0.05:
                        bad_performance_stocks.append((self.datas[i], self.context[i].profit_rate))

        if to_open_stock_count + holding_stock_count > 100:
            # sort bad performance stocks by profit rate
            bad_performance_stocks.sort(key=lambda x: x[1])
            i = 0
            for stock_id, profit_rate in bad_performance_stocks:
                if i >= to_open_stock_count:
                    break

                self.order_target_percent(stock_id, target=0.0)
                self.context[i].order_state = OrderState.ORDER_CLOSING
                i += 1
