    ...
    def stop(self):
        ...
        for data, position in self.positions.items():
            if position:
                ...
                print(
                    f"""{data._name}: 持仓股数: {"%.2f" % position.size},
 成本: {self.context[i].open_price}, 止损: {self.context[i].stop_price}"""
                )
...
if __name__ == "__main__":
    ...
    run(
        strategy_name="trend_follow_etf_5",
        cerebro=cerebro,
        stocks=stocks,
...
if __name__ == "__main__":
    ...
    run(strategy_name='trend_follow_etf_5', cerebro=cerebro, stocks=stocks,
