...
from indicator_rsrs import RSRS, RSRS_Norm  # noqa: E402
...
class RsrsNormStrategy(MyStrategy):
    ...
    def next(self):
        ...
        if not self.position:  # not in the market
            ...
            if (self.rsrs_norm.beta_right[0] > 0.7
...
if __name__ == '__main__':
    ...
    if os.environ.get('OPTIMIZER') == 'True':
        strats = cerebro.optstrategy(RsrsNormStrategy,
                                     period=range(1, 10))
    else:
        cerebro.addstrategy(RsrsNormStrategy)

    strategy_name = os.path.splitext(os.path.basename(__file__))[0]
    run(strategy_name=strategy_name, cerebro=cerebro, stocks=RsrsNormStrategy.stocks,
