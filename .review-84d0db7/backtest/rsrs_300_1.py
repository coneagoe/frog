...
from .indicator_rsrs_0 import RsrsZscore
...
class RsrsStrategy(MyStrategy):
    ...
    def next(self):
        ...
        if not self.position:
            if rsrs_value > self.p.S and self.context[0].current_price > self.ma20[0]:
                self.order_target_percent(self.data, 0.99)  # enter long
                # self.buy()
        else:
            if rsrs_value < -self.p.S:
                self.close()
...
if __name__ == "__main__":
    ...
    if os.environ.get("OPTIMIZER") == "True":
        strats = cerebro.optstrategy(RsrsStrategy, period=range(1, 10))
    else:
