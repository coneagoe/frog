...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="从 DB 读取社保基金持股比例变化因子面板并用 Alphapurify 做分析")
...
def load_ssf_ratio_change_panel_from_db(
    storage: Any,
    start_date: str | None,
    end_date: str | None,
    max_stocks: int,
    ...
    # SSF_FACTOR_RATIO_CHANGE now matches the downstream expected name, so no renaming needed
    # but keep the logic to ensure the factor column exists
    if SSF_FACTOR_RATIO_CHANGE not in df.columns:
        raise ValueError(f"Expected factor column '{SSF_FACTOR_RATIO_CHANGE}' not found in panel")

    # validate required panel columns before selecting
    required_cols = ["datetime", "symbol", "close", SSF_FACTOR_RATIO_CHANGE]
    missing_cols = [c for c in required_cols if c not in df.columns]
    if missing_cols:
        raise ValueError(
            f"Missing required panel columns: {missing_cols}. "
            f"FactorAnalyzer requires datetime, symbol, close, and {SSF_FACTOR_RATIO_CHANGE}"
        )
...
def main(argv: list[str] | None = None, storage: Any | None = None) -> int:
    ...
    try:
        panel_df = load_ssf_ratio_change_panel_from_db(
            storage=storage,
        ...
        if panel_df.empty:
            print("[错误] 未从 DB 构造出有效的社保基金持股比例变化因子面板数据")
            return EXIT_ERROR
