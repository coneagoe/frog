...
def _normalize_history_df(
    ...
    normalized = compute_momentum(normalized, n=momentum_n)
    normalized = normalized.dropna(subset=[COL_MOMENTUM]).copy()
    if normalized.empty:
        return pd.DataFrame()
    ...
    return normalized.loc[:, ["datetime", "symbol", "close", COL_MOMENTUM]].copy()


def load_momentum_panel_from_db(
    storage: Any,
    start_date: str | None,
    end_date: str | None,
    max_stocks: int,
...
def main(argv: list[str] | None = None, storage: Any | None = None) -> int:
    ...
    try:
        panel_df = load_momentum_panel_from_db(
            storage=storage,
