...
from utility import retry_async
...
headers = {
    "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2227.0 Safari/537.36"  # noqa: E501
}
...
class TianTianCrawler(object):
    ...
    async def _download_history_netvalues(
        ...
        async with aiohttp.ClientSession(headers=headers, timeout=timeout) as session:
            tasks = [
                self._fetch_history_netvalues(session, fund_id, start_date, end_date, i)
    ...
    async def _save_general_info(self, fund_id, page):
        ...
        for th in soup.find_all("th"):
            ...
            if th.text == "发行日期":
                Y, M, D, tmp = re.split("年|月|日", th.next_sibling.text)
                launch_date = f"{Y}-{M}-{D}"
                continue
    ...
    async def _save_managers(self, fund_id, page):
        ...
        document = {"fund_id": fund_id, "managers": df.iloc[0:].values.tolist()}
    ...
    async def _save_history_scales(self, fund_id: str, page):
        df = pd.read_html(page, encoding="utf-8")[0]
        document = {"fund_id": fund_id, "scales": df.iloc[0:].values.tolist()}
