...
class StockMonitorTarget(Base):
    ...
    market = Column(String(5), nullable=False, default="A", server_default="A", comment="市场: A / HK / ETF")
    condition = Column(JSONB, nullable=False, comment="触发条件JSON")
    note = Column(Text, nullable=True, comment="用户备注")
    frequency = Column(String(10), nullable=False, default="daily", server_default="daily", comment="监控频率: daily / intraday")
    reset_mode = Column(String(10), nullable=False, default="auto", server_default="auto", comment="重置模式: auto / manual")
    enabled = Column(Boolean, nullable=False, default=True, server_default=text("true"), comment="是否启用")
    last_state = Column(Boolean, nullable=False, default=False, server_default=text("false"), comment="上次条件是否成立（边沿触发用）")
